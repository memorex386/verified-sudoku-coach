...
4. Register the provider-profile ID/digest, prompt/schema hashes, requested model, exact native
   settings, output bound, `responseStoragePolicy:"request-not-to-store"`, separate abuse-monitoring
   and verified retention-control evidence, `automaticRetry:false`, visible failure policy, and
   eval-suite version in `ai/runtime-manifest.json`.
...
acceptance freezes the complete behavior identity. A mutable hosted-model route cannot become the
reviewer-cohort registration; use an immutable provider revision or a digest-bound open-weight
artifact identity.
