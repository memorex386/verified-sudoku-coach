...
const rotate = (word: number, count: number): number => (word >>> count) | (word << (32 - count));

/** SHA-256 over bytes, with no host crypto, clock, or platform API. */
export function sha256(bytes: Uint8Array): string {
  if (!(bytes instanceof Uint8Array)) throw new Error("hash-bytes");
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (const [i, word] of [a, b, c, d, e, f, g, h].entries()) state[i] = (state[i]! + word) >>> 0;
  }
...
export function fingerprint(projection: string, value: unknown): Fingerprint {
  if (typeof projection !== "string" || (projection !== "fixture-digit-d4" &&
    !/^[a-z]+(?:-[a-z]+)*(?![\s\S])/.test(projection))) throw new Error("hash-projection");
